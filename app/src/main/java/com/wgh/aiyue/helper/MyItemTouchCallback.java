...
public class MyItemTouchCallback extends ItemTouchHelper.Callback {
    ...
    public boolean onMove(RecyclerView recyclerView, RecyclerView.ViewHolder viewHolder, RecyclerView.ViewHolder target) {
        int fromPosition = viewHolder.getAdapterPosition(); // to get the position of ViewHolder.
        int toPosition = target.getAdapterPosition();       // to get the position of target ViewHolder.
    ...
    public void onChildDraw(Canvas c, RecyclerView recyclerView, RecyclerView.ViewHolder viewHolder, float dX, float dY, int actionState, boolean isCurrentlyActive) {
        if (actionState == ItemTouchHelper.ACTION_STATE_SWIPE) {
            // to change the transparent while sliding.
